...
1. [HSMR_TableA1a_Automated](https://github.com/NHS-NSS-transforming-publications/automating-excel/raw/master/HSMR_TableA1a_automated.xls)
...
* If R is used in the data wrangling/analysis process then the `openxlsx` package can be used to automitcally populate the data tab, again decreasing manual steps in the workflow.
...
* The 'dynamic ranges' formula in Excel will be key when creating charts which update automatically, so it will be useful to be familiar with this.
* `VLOOKUP` will be used to search data and create cell content.
* In order to create drop-down selection, we recommend using an alternative method to macros by clicking: Developer > Insert > Form Controls > Combo Box (see ['HSMR_TableA1a_automated.xls'](https://github.com/NHS-NSS-transforming-publications/automating-excel/raw/master/HSMR_TableA1a_automated.xls) for examples.)
